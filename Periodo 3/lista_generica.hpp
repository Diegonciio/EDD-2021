...
// lista generica
//
// Created by Diego Galdámez GF100516
...
class Lista{
	...
	public:
		...
		Lista();
		~Lista();
		Lista(const Lista &otra); // Cpnstruir una lista a partir de otra en una dirección de memoria
		bool estaVacia();
		int tamano();
...
//Constructor con tamño por defecto, en este caso 4
template <typename TIPODATO>
...
//Construir una lista a aprtir de otra
template<typename TIPODATO>
Lista<TIPODATO>::Lista(const Lista<TIPODATO> &otra){
//	cout << "Copiando una lista de otra... " << endl;
...
// Valor del elemento en la posición índice
template <typename TIPODATO>
...
TIPODATO Lista<TIPODATO>::remover(int indice){
	//validar el indice
	if(this->estaVacia()) throw "No se puede remover elemento de una lista vacia";
	if(indice < 0 || indice >= this->cuenta) throw "Indice fuera de rango";
	
//	cout << "Removiendo el elemento con indice " << indice << "..." << endl;
	// Lee el valor en el indice y desplaza elemento a la izquierda
	TIPODATO valor = this->items[indice];
	for(int i = indice; i < cuenta - 1; i++){
//		cout << " Deplazando elemento " << items[i] << " del indice";
